#!/usr/bin/env python3
"""
predict_positions_from_tracking_first_npy.py (FIXED TOP/BOTTOM ORDERING)
...
Each dict is expected to contain:
  - "keypoints": (17,3) array-like (COCO: x,y,score)
  - "bbox": [x1,y1,x2,y2,score] or similar
  - "track_id": int (optional)

Then:
  - selects 2 athletes per frame
  - orders them consistently using a chosen strategy
  - normalizes using bjjtrack.jiujitsu.utils.normalize
  - predicts position using classifier.pickle
  - saves CSV + JSON summaries

This script does NOT compute accuracy (no ground truth).
"""
...
def mean_kpt_y(kpts, idxs, min_kpt_score=0.20):
    """
    Compute mean y over selected keypoints, ignoring low-confidence points.
    kpts: (17,3) [x,y,score]
    """
...
def vertical_role_score(person, role_mode="tb", use_pose=True, min_kpt_score=0.20):
    """
    Returns a vertical score where larger means "lower in image" (more bottom).
    If use_pose: use hips+shoulders y (more stable than bbox under overlap).
    Otherwise: bbox center y.
    """
    # Pose-based score (preferred)
    if use_pose:
        k = person.get("keypoints", None)
        if k is not None:
            # COCO indices: shoulders 5,6 ; hips 11,12
            y_hips = mean_kpt_y(k, [11, 12], min_kpt_score=min_kpt_score)
            y_sh = mean_kpt_y(k, [5, 6], min_kpt_score=min_kpt_score)
            ys = [y for y in [y_hips, y_sh] if y is not None]
            if ys:
                return float(np.mean(ys))

    # Fallback: bbox center y
    bb = person.get("bbox", [0, 0, 0, 0, 0])
    return bbox_center_y(bb)
...
def order_two(a, b, order_mode="tb", use_pose_for_tb=True, min_kpt_score=0.20):
    """
    Ordering modes:
      - "trackid": by track_id (ascending), else left-to-right
      - "ltr": left-to-right by bbox center x
      - "tb": TOP first, BOTTOM second (top has smaller y, bottom has larger y)
      - "bt": BOTTOM first, TOP second
    """
    order_mode = (order_mode or "tb").lower().strip()
    ...
    if order_mode in ("tb", "bt"):
        ya = vertical_role_score(a, use_pose=use_pose_for_tb, min_kpt_score=min_kpt_score)
        yb = vertical_role_score(b, use_pose=use_pose_for_tb, min_kpt_score=min_kpt_score)

        # y larger => lower in image => "bottom"
        top, bottom = (a, b) if ya <= yb else (b, a)
...
def main():
    ...
    # IMPORTANT: use tb ordering instead of left-right or track_id
    ap.add_argument("--order-mode", default="tb",
                    choices=["tb", "bt", "ltr", "trackid"],
                    help="How to order the two athletes. "
                         "tb=TOP first, BOTTOM second (recommended for your 1/2 definition).")
    ap.add_argument("--tb-use-pose", action="store_true",
                    help="Use hips/shoulders keypoint y (more stable) for top/bottom ordering.")
    ap.add_argument("--tb-min-kpt-score", type=float, default=0.20,
                    help="Min keypoint score to use in top/bottom ordering when --tb-use-pose is set.")

    ap.add_argument("--order-invariant", action="store_true",
                    help="Average probs across (p1,p2) and (p2,p1). "
                         "NOTE: if you want strict top/bottom meaning for 1/2, do NOT enable this.")
    ...
    for _, path in enumerate(npy_files):
        ...
        pA, pB = order_two(
            ...
            use_pose_for_tb=bool(args.tb_use_pose),
            min_kpt_score=float(args.tb_min_kpt_score),
        )
    ...
    summary = {
        ...
        "order_mode": args.order_mode,
        "tb_use_pose": bool(args.tb_use_pose),
        "tb_min_kpt_score": float(args.tb_min_kpt_score),
