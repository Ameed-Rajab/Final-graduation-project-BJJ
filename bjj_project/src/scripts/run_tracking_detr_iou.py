...
def main():
    ...
    while True:
        ...
        if dets.size:
            ...
            # --- Reject "merged" detections that are too large (often covers both athletes) ---
        max_area_frac = 0.45  # 45% of the frame; tune 0.35..0.55
        frame_area = float(w * h)
        areas = (dets[:, 2] - dets[:, 0]) * (dets[:, 3] - dets[:, 1])
        dets = dets[(areas / frame_area) <= max_area_frac]
