...
# ----------------------------
# Upgraded tracker (2 players): motion + global assignment + gating
# ----------------------------
...
class MotionIoUTracker:
    """A stronger drop-in replacement for the greedy IoU tracker.

    - Predicts next bbox using constant-velocity motion on center.
    - Uses global assignment (best total IoU) for up to 2 tracks.
    - Rejects huge merged detections (area gating).
    - Rejects matches that jump too far (center jump gating).

    Interface is identical to IoUTracker.update():
    ...
    def _is_merged_box(self, det: np.ndarray) -> bool:
        if self.frame_w is None or self.frame_h is None:
            return False
        a = bbox_area(det)
        return (a / float(self.frame_w * self.frame_h)) > self.max_area_frac

    def _too_far_jump(self, t: Track, det: np.ndarray) -> bool:
        if self.frame_w is None or self.frame_h is None:
            return False
        tcx, tcy = bbox_center(t.bbox)
        dcx, dcy = bbox_center(det)
        dist = ((tcx - dcx) ** 2 + (tcy - dcy) ** 2) ** 0.5
        diag = math.sqrt(self.frame_w ** 2 + self.frame_h ** 2)
    ...
    def update(self, detections: np.ndarray) -> np.ndarray:
        ...
        # filter merged huge detections
        dets = np.asarray([d for d in dets if not self._is_merged_box(d)], dtype=np.float32)
        if dets.size == 0:
            for t in self.tracks:
                t.lost += 1
            self.tracks = [t for t in self.tracks if t.lost <= self.max_lost]
        ...
        if len(self.tracks) == 0:
            # assume dets already sorted by score (caller does that)
            for j in range(min(self.max_tracks, dets.shape[0])):
                t = Track(track_id=self._next_id, bbox=dets[j].copy(), lost=0)
                t.prev_center = bbox_center(t.bbox)
                self._next_id += 1
        ...
        # global assignment (bruteforce) for up to 2 tracks
        best_sum = -1.0
        best_assign = None  # list of length M mapping track->det or -1
        ...
        for perm in itertools.permutations(det_indices, min(M, N)):
            ...
            for i in range(M):
                ...
                if j == -1:
                    continue
                if self._too_far_jump(self.tracks[i], dets[j]):
                    ok = False
                    break
                iou = iou_xyxy(preds[i], dets[j])
                if iou < self.iou_thr:
                    ok = False
                    break
                s += iou
        ...
        if best_assign is not None:
            for i, t in enumerate(self.tracks):
                ...
                if j == -1:
                    ...
                    continue
                det = dets[j]
                c_prev = t.prev_center if t.prev_center is not None else bbox_center(t.bbox)
                c_now = bbox_center(det)
        ...
        # spawn new tracks if we have fewer than max_tracks
        for j in range(N):
            if j in used_det:
                continue
...
def parse_args():
    ...
    p.add_argument("--tracker-max-area-frac", type=float, default=0.45,
                   help="Reject detections whose area exceeds this fraction of frame area (merged boxes)")
    p.add_argument("--tracker-max-center-jump-frac", type=float, default=0.20,
                   help="Reject matches that jump more than this fraction of frame diagonal")
...
def main():
    ...
    # Upgraded tracker (motion + global assignment + gating)
    tracker = MotionIoUTracker(
        iou_thr=args.tracker_iou,
        max_lost=args.tracker_max_lost,
        max_tracks=2,
