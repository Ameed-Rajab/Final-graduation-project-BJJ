...
# ----------------------------
# Upgraded tracker (for ~2 players)
# - Global assignment (avoids greedy mistakes)
# - Simple constant-velocity motion prediction
# - Gating against huge merged boxes and sudden jumps
# ----------------------------
@dataclass
class Track:
    track_id: int
...
def bbox_center_xyxy(b: np.ndarray) -> Tuple[float, float]:
    x1, y1, x2, y2 = map(float, b[:4])
    return (0.5 * (x1 + x2), 0.5 * (y1 + y2))


def bbox_area_xyxy(b: np.ndarray) -> float:
    x1, y1, x2, y2 = map(float, b[:4])
    return max(1.0, (x2 - x1)) * max(1.0, (y2 - y1))


class MotionIoUTracker:
    """A lightweight but much more stable tracker than greedy IoU.

    Designed for the common BJJ case where we track ~2 people.
    """
    ...
    def __init__(
        ...
        max_center_jump_frac: float = 0.20,
    ):
        self.iou_thr = float(iou_thr)
        self.max_lost = int(max_lost)
        self.max_tracks = int(max_tracks)

        # reject detections that occupy too much of the frame (often "merged" boxes)
        self.max_area_frac = float(max_area_frac)

        # reject matches where the detection center jumps too far (fraction of frame diagonal)
    ...
    def _valid_det(self, det: np.ndarray) -> bool:
        if self.frame_w is None or self.frame_h is None:
            return True
        area = bbox_area_xyxy(det)
        return (area / float(self.frame_w * self.frame_h)) <= self.max_area_frac

    def _predict_bbox(self, t: Track) -> np.ndarray:
        # constant-velocity prediction on bbox center
        b = t.bbox.copy()
    ...
    def _ok_center_jump(self, t: Track, det: np.ndarray) -> bool:
        if self.frame_w is None or self.frame_h is None:
            return True
        import math
        diag = math.sqrt(self.frame_w * self.frame_w + self.frame_h * self.frame_h)
        tcx, tcy = bbox_center_xyxy(t.bbox)
        dcx, dcy = bbox_center_xyxy(det)
    ...
    def update(self, detections: np.ndarray) -> np.ndarray:
        ...
        # filter merged/huge detections (kills many top-down pose failures)
        dets = np.asarray([d for d in dets if self._valid_det(d)], dtype=np.float32)
        if dets.size == 0:
            for t in self.tracks:
                t.lost += 1
            self.tracks = [t for t in self.tracks if t.lost <= self.max_lost]
        ...
        # If no tracks yet -> start from top detections
        if len(self.tracks) == 0:
            for j in range(min(self.max_tracks, dets.shape[0])):
                ...
                new.prev_center = bbox_center_xyxy(new.bbox)
                self._next_id += 1
                self.tracks.append(new)
            return np.asarray([[t.track_id, *t.bbox[:5]] for t in self.tracks], dtype=np.float32)

        # predicted boxes for matching
        preds = [self._predict_bbox(t) for t in self.tracks]
        M, N = len(self.tracks), dets.shape[0]

        # For 2 players (max_tracks=2), brute-force best global assignment.
        # For M>2, this still works but grows factorially; we keep M small.
        import itertools

        best_sum = -1.0
        best_assign = [-1] * M  # det index per track, or -1

        det_indices = list(range(N))
        for perm in itertools.permutations(det_indices, min(M, N)):
            assign = [-1] * M
            ...
            for i in range(M):
                ...
                if j == -1:
                    continue
                if not self._ok_center_jump(self.tracks[i], dets[j]):
                    ok = False
                    break
                cur_iou = iou_xyxy(preds[i], dets[j])
        ...
        if best_sum >= 0:
            for i, t in enumerate(self.tracks):
                ...
                det = dets[j]
                c_prev = t.prev_center if t.prev_center is not None else bbox_center_xyxy(t.bbox)
                c_now = bbox_center_xyxy(det)
                t.velocity = (c_now[0] - c_prev[0], c_now[1] - c_prev[1])
                t.prev_center = c_now

                t.bbox = det
                t.last_score = float(det[4])
                t.lost = 0
                used_det.add(j)
        else:
            # no valid assignment -> mark lost
            for t in self.tracks:
                t.lost += 1
        ...
        # spawn new tracks if we have room
        for j in range(N):
            if j in used_det:
                continue
            if len(self.tracks) >= self.max_tracks:
                break
            det = dets[j]
            new = Track(track_id=self._next_id, bbox=det, last_score=float(det[4]), lost=0)
            new.prev_center = bbox_center_xyxy(new.bbox)
            self._next_id += 1
            self.tracks.append(new)
...
def main():
    ...
    tracker = MotionIoUTracker(iou_thr=args.tracker_iou, max_lost=args.tracker_max_lost, max_tracks=2,
                            max_area_frac=args.tracker_max_area_frac,
                            max_center_jump_frac=args.tracker_max_center_jump_frac)
...
if __name__ == "__main__":
    main()
    p.add_argument("--tracker-max-area-frac", type=float, default=0.45,
                   help="Reject detections whose bbox area exceeds this fraction of the frame (prevents merged boxes)")
    p.add_argument("--tracker-max-center-jump-frac", type=float, default=0.20,
                   help="Reject matches if bbox center jumps more than this fraction of frame diagonal")
